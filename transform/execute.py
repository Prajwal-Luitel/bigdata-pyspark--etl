...
def create_spark_session(logger):
    ...
    logger.debug("Create spark session")
...
def create_master_table(logger, output_dir, artists_df, recommendations_df, tracks_df):
    ...
    logger.debug("Creating master table")
    ...
    logger.info("Stage 2: Master tabel saved")

def create_query_tables(logger,output_dir, artists_df, recommendations_df, tracks_df):
    """Stages 3: Create query" optimized tables"""
    logger.debug("creating query tables")
    ...
    tracks_exploded = tracks_df.withColumn("id_artists_array", F.from_json(F.col("id_artists"), T.ArrayType(T.StringType()))).withColumn("artist_id", F.explode("id_artists_array")) \
                                .select("id","artist_id")
